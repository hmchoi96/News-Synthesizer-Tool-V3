...
def generate_full_report(topic, industry, country, language="English", user_comment=""):
    ...
    # 1. Big Picture
    big_prompt = get_big_picture_prompt(topic, industry, country, current_date, language)
    big_result = call_gpt(prompt=big_prompt, model="gpt-4o")

    # 2. Mid Picture
    mid_prompt = get_mid_picture_prompt(topic, industry, country, current_date, language)
    mid_result = call_gpt(prompt=mid_prompt, model="gpt-4o")

    # 3. Small Picture
    small_prompt = get_small_picture_prompt(topic, industry, country, current_date, language)
